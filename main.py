...
# Import Supabase configuration if available
try:
    if Config.ENABLE_SUPABASE:
        from supabase_config import supabase, test_supabase_connection
    else:
        supabase = None
...
class User(db.Model):
    ...
    coins = db.Column(db.Float, default=10.0)  # Changed to Float for decimal support
...
class CoinTransaction(db.Model):
    ...
    amount = db.Column(db.Float)  # Changed to Float for decimal support - positive for credit, negative for debit
    description = db.Column(db.String(200))
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
...
class HelpVideo(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    title = db.Column(db.String(200), nullable=False)
    description = db.Column(db.Text)
    video_url = db.Column(db.String(500), nullable=False)  # YouTube URL
    video_type = db.Column(db.String(50), default='tutorial')  # tutorial, guide, announcement
    display_order = db.Column(db.Integer, default=0)
    is_active = db.Column(db.Boolean, default=True)
    show_on_dashboard = db.Column(db.Boolean, default=False)
    thumbnail_url = db.Column(db.String(500))
    duration = db.Column(db.String(20))  # e.g., "5:30"
...
class PushNotification(db.Model):
    ...
    notification_type = db.Column(db.String(50), default='general')  # general, announcement, update
    target_users = db.Column(db.String(20), default='all')  # all, active, premium
    is_sent = db.Column(db.Boolean, default=False)
    send_at = db.Column(db.DateTime)  # For scheduled notifications
    sent_at = db.Column(db.DateTime)
    click_url = db.Column(db.String(500))  # Optional URL to open when clicked
...
def update_order_from_jap(order):
    """Update order details from JAP API with improved status mapping"""
    ...
    try:
        ...
        if not jap_status:
            print(f"Failed to get JAP status for order {order.id}")
            return False

        print(f"JAP API Response for Order {order.id}: {jap_status}")

        # Improved status mapping with exact JAP API status names
        status_mapping = {
            'Pending': 'Pending',
            ...
            'Partial': 'Partial',
            'Completed': 'Completed',
            'Complete': 'Completed',  # Some APIs use 'Complete' instead of 'Completed'
            'Canceled': 'Cancelled',
            'Cancelled': 'Cancelled',
            'Failed': 'Failed',
            'Error': 'Failed'
        ...
        # Update order details with better data handling
        old_status = order.status
        order.status = new_status
        
        # Update counts with proper type conversion
        start_count = jap_status.get('start_count', 0)
        remains = jap_status.get('remains', 0)
        
        if isinstance(start_count, (int, float, str)):
            try:
                order.start_count = int(float(start_count))
            except (ValueError, TypeError):
                order.start_count = 0
        
        if isinstance(remains, (int, float, str)):
            try:
                order.remains = int(float(remains))
            except (ValueError, TypeError):
                order.remains = 0
        ...
        if new_status == 'Completed' and old_status != 'Completed':
            ...
            print(f"Order {order.id} marked as completed at {order.completion_time}")
...
def sync_user_data_to_supabase(user_id):
    ...
    if not supabase or not Config.ENABLE_SUPABASE:
        return True  # Return True to not break the flow when Supabase is disabled
    ...
    try:
        ...
        try:
            ...
            print(f"Error getting ad session for user {user_id}: {e}")
            ads_watched_today = 0
            direct_ads_watched_today = 0
        
        # Prepare data for Supabase with proper decimal handling
        user_data = {
            'user_id': user.id,
            'username': user.username,
            'email': user.email,
            'coins': float(user.coins),  # Ensure decimal coins are properly handled
            ...
            'device_type': 'browser',  # Default, will be updated based on actual usage
            'total_earnings': float(user.coins),  # Track total earnings
            'last_ad_watch': datetime.utcnow().isoformat() if ads_watched_today > 0 else None
        }
        
        # Upsert to Supabase with better error handling
        result = supabase.table('user_activity').upsert(user_data, on_conflict='user_id').execute()
        
        # Also sync to admin activity table for live monitoring
        admin_data = {
            'timestamp': datetime.utcnow().isoformat(),
            'user_id': user.id,
            'activity_type': 'sync',
            'details': f'User {user.username} synced with {ads_watched_today} ads watched today'
        }
        supabase.table('admin_activity_log').insert(admin_data).execute()
...
def check_user_ad_activity(user_id, days=None):
    """Check if user has watched ads in the last X days to qualify for login bonus"""
    try:
        # Get configuration values
        if days is None:
            activity_days_config = CoinConfiguration.query.filter_by(config_name='login_bonus_activity_days', is_active=True).first()
            days = activity_days_config.coin_value if activity_days_config else 3
        
        min_ads_config = CoinConfiguration.query.filter_by(config_name='min_ads_for_login_bonus', is_active=True).first()
        min_ads_required = min_ads_config.coin_value if min_ads_config else 1
        
        # Calculate date range
        end_date = datetime.now()
        start_date = end_date - timedelta(days=days)
        
        # Check for ad watching activity in the last X days
        ad_sessions = AdWatchSession.query.filter(
            AdWatchSession.user_id == user_id,
            AdWatchSession.created_at >= start_date
        ).all()
        
        # Check if user has watched minimum required ads in the specified period
        total_ads_watched = 0
        for session in ad_sessions:
            total_ads_watched += session.ads_watched_count
            try:
                total_ads_watched += session.direct_ads_watched_count
            except AttributeError:
                pass  # Handle missing column gracefully
        
        # Also check for coin transactions from ad watching (alternative check)
        ad_transactions = CoinTransaction.query.filter(
            CoinTransaction.user_id == user_id,
            CoinTransaction.transaction_type.in_(['ad_reward', 'direct_ad_reward']),
            CoinTransaction.created_at >= start_date
        ).count()
        
        # User qualifies if they have watched minimum required ads OR have sufficient ad reward transactions
        return total_ads_watched >= min_ads_required or ad_transactions >= min_ads_required
        
    except Exception as e:
        print(f"Error checking user ad activity: {e}")
        # If there's an error, be conservative and don't allow login bonus for new users
        # But allow for existing users to prevent service disruption
        user = User.query.get(user_id)
        if user and user.id <= 100:  # Assuming first 100 users are existing users
            return True
        return False
...
def register():
    ...
    if request.method == 'POST':
        ...
        reg_transaction = CoinTransaction(
            ...
            description=f'Welcome bonus for new registration (one-time only)'
        )
    ...
    if request.method == 'POST':
        ...
        if user and check_password_hash(user.password, password):
            ...
            if user.last_login_date != today:
                # Check if user has watched ads recently to qualify for login bonus
                has_recent_activity = check_user_ad_activity(user.id)
                
                if has_recent_activity:
                    # Detect if login is from webview app or browser
                    user_agent = request.headers.get('User-Agent', '').lower()
                    is_webview = 'webview' in user_agent or 'mobile app' in user_agent
                    
                    # Get login bonus based on platform
                    if is_webview:
                        login_bonus = 5  # Mobile app login bonus
                        platform_text = "mobile app"
                    else:
                        login_bonus = 2  # Browser login bonus
                        platform_text = "browser"
                    ...
                    transaction = CoinTransaction(
                        ...
                        amount=login_bonus,
                        description=f'Daily login bonus ({platform_text})'
                    )
                    db.session.add(transaction)
                    flash(f'Welcome back, {user.username}! You earned {login_bonus} coins for {platform_text} login!')
                else:
                    # Update login date but don't give bonus
                    user.last_login_date = today
                    flash(f'Welcome back, {user.username}! Watch ads to qualify for daily login bonus.')
            ...
            # Sync user data to Supabase on login
            sync_user_data_to_supabase(user.id)
...
def order_history():
    ...
    # Auto-update all incomplete orders from JAP API (more frequent updates)
    updated_count = 0
    for order in orders:
        if order.status in ['Pending', 'Processing', 'Partial'] and order.jap_order_id:
            # Update if last update was more than 2 minutes ago (reduced from 5 minutes)
            if not order.last_status_update or (datetime.utcnow() - order.last_status_update).total_seconds() > 120:
                if update_order_from_jap(order):
                    updated_count += 1

    # Show update message if any orders were updated
    if updated_count > 0:
        flash(f'✅ Updated {updated_count} order(s) from JAP API')
...
@app.route('/api/get-user-ad-session')
def api_get_user_ad_session():
    """API endpoint to get user's current ad session data"""
    if 'user_id' not in session:
        return jsonify({'success': False, 'message': 'Please log in first.'}), 401
    ...
    try:
        ...
        session_data = {
            ...
            'ads_watched': ad_session.ads_watched_count if ad_session else 0,
            'direct_ads_watched': getattr(ad_session, 'direct_ads_watched_count', 0) if ad_session else 0,
            'cooldown_until': ad_session.cooldown_until.isoformat() if ad_session and ad_session.cooldown_until else None,
            'direct_cooldown_until': getattr(ad_session, 'direct_cooldown_until', None),
            'last_ad_time': ad_session.last_ad_time.isoformat() if ad_session and ad_session.last_ad_time else None,
            'last_direct_ad_time': getattr(ad_session, 'last_direct_ad_time', None)
        }

        # Convert direct cooldown to string if exists
        if session_data['direct_cooldown_until']:
            try:
                session_data['direct_cooldown_until'] = session_data['direct_cooldown_until'].isoformat()
            except:
                session_data['direct_cooldown_until'] = None

        # Convert direct ad time to string if exists
        if session_data['last_direct_ad_time']:
            try:
                session_data['last_direct_ad_time'] = session_data['last_direct_ad_time'].isoformat()
            except:
                session_data['last_direct_ad_time'] = None
...
@app.route('/api/get-rendered-ads')
def api_get_rendered_ads():
    """API endpoint to get rendered HTML for both ad players"""
    try:
        import random
        
        # Get active multiple ads for both players
        watch_ad_1_ads = MultipleAdConfig.query.filter_by(ad_type='watch_ad_1', is_active=True).all()
        watch_ad_2_ads = MultipleAdConfig.query.filter_by(ad_type='watch_ad_2', is_active=True).all()
        
        # Select random ads if available
        ad_1_html = None
        ad_2_html = None
        
        if watch_ad_1_ads:
            selected_ad_1 = random.choice(watch_ad_1_ads)
            ad_1_html = f"""
<!DOCTYPE html>
<html>
<head>
    ...
    <meta http-equiv="Content-Security-Policy" content="default-src * data: blob: 'unsafe-inline' 'unsafe-eval'; script-src * data: blob: 'unsafe-inline' 'unsafe-eval'; connect-src * data: blob: 'unsafe-inline'; img-src * data: blob: 'unsafe-inline'; frame-src *; style-src * data: blob: 'unsafe-inline';">
    <style>
        * {{
            margin: 0;
            padding: 0;
    ...
    <script>
        ...
        (function() {{
            try {{
                ...
                // Notify parent that ad is loaded
                setTimeout(function() {{
                    if (window.parent) {{
                ...
                console.log('Ad script execution:', e);
            }}
        }})();
        
        // Ensure all external scripts load properly
...
"""
        ...
        if watch_ad_2_ads:
            selected_ad_2 = random.choice(watch_ad_2_ads)
            ad_2_html = f"""
<!DOCTYPE html>
<html>
<head>
...
"""
        ...
        return jsonify({
            'success': False,
            'message': f'Error rendering ads: {str(e)}'
        }), 500

# New Ad Management System
class AdNetwork(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    network_name = db.Column(db.String(100), nullable=False)  # Propeller, Adsterra, Hilltop
    ad_type = db.Column(db.String(50), nullable=False)  # popunder, banner, native_banner
    ad_code = db.Column(db.Text, nullable=False)
    ad_url = db.Column(db.String(500))  # For popunder direct URLs
    is_active = db.Column(db.Boolean, default=True)
    display_order = db.Column(db.Integer, default=0)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    updated_at = db.Column(db.DateTime, default=datetime.utcnow)
...
class UserSession(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('user.id'))
    device_type = db.Column(db.String(50), default='browser')  # browser, webview
    session_start = db.Column(db.DateTime, default=datetime.utcnow)
    is_active = db.Column(db.Boolean, default=True)
    user_agent = db.Column(db.String(500))
    ip_address = db.Column(db.String(50))

class UserEarningStats(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('user.id'))
    date = db.Column(db.String(20))  # YYYY-MM-DD format
    total_coins_earned = db.Column(db.Float, default=0.0)
    ads_watched = db.Column(db.Integer, default=0)
    login_bonus = db.Column(db.Float, default=0.0)
    referral_bonus = db.Column(db.Float, default=0.0)
    spin_wheel_earnings = db.Column(db.Float, default=0.0)
    coin_purchases = db.Column(db.Float, default=0.0)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    updated_at = db.Column(db.DateTime, default=datetime.utcnow)

class AdPerformanceStats(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    ad_network_id = db.Column(db.Integer, db.ForeignKey('ad_network.id'))
    date = db.Column(db.String(20))  # YYYY-MM-DD format
...
@app.route('/api/get-ad-codes-new')
def api_get_ad_codes_new():
    """API endpoint to get ads in specific sequence: popunder -> banner -> native_banner"""
    try:
        import random
        
        # Get ads by type with priority order
        popunder_ads = AdNetwork.query.filter_by(ad_type='popunder', is_active=True).all()
        banner_ads = AdNetwork.query.filter_by(ad_type='banner', is_active=True).all()
        native_banner_ads = AdNetwork.query.filter_by(ad_type='native_banner', is_active=True).all()
        
        selected_ads = []
        
        # Step 1: Select popunder ad (highest priority)
        if popunder_ads:
            selected_popunder = random.choice(popunder_ads)
            selected_ads.append({
                'id': selected_popunder.id,
                'network_name': selected_popunder.network_name,
                'ad_type': 'popunder',
                'ad_url': selected_popunder.ad_url if selected_popunder.ad_url else None,
                'ad_code': selected_popunder.ad_code,
                'step': 1,
                'open_in_new_tab': True  # Popunders should open in new tab
            })
        
        # Step 2: Select banner ad
        if banner_ads:
            selected_banner = random.choice(banner_ads)
            selected_ads.append({
                'id': selected_banner.id,
                'network_name': selected_banner.network_name,
                'ad_type': 'banner',
                'ad_url': selected_banner.ad_url if selected_banner.ad_url else None,
                'ad_code': selected_banner.ad_code,
                'step': 2,
                'open_in_new_tab': False  # Banners show inline
            })
        
        # Step 3: Select native banner ad
        if native_banner_ads:
            selected_native = random.choice(native_banner_ads)
            selected_ads.append({
                'id': selected_native.id,
                'network_name': selected_native.network_name,
                'ad_type': 'native_banner',
                'ad_url': selected_native.ad_url if selected_native.ad_url else None,
                'ad_code': selected_native.ad_code,
                'step': 3,
                'open_in_new_tab': False  # Native banners show inline
            })
        
        # If we don't have enough ads, fill with any available ads
        if len(selected_ads) < 3:
            all_remaining_ads = AdNetwork.query.filter_by(is_active=True).all()
            # Remove already selected ads by ID
            selected_ids = [ad['id'] for ad in selected_ads]
            remaining_ads = [ad for ad in all_remaining_ads if ad.id not in selected_ids]
            ...
            for ad in remaining_ads:
                ...
                selected_ads.append({
                    ...
                    'ad_type': ad.ad_type,
                    'ad_url': ad.ad_url if ad.ad_url else None,
                    'ad_code': ad.ad_code,
                    'step': len(selected_ads) + 1,
                    'open_in_new_tab': ad.ad_type == 'popunder'
                })
        
        if len(selected_ads) == 0:
            return jsonify({
                'success': False,
                'message': 'No active ads configured. Please add ads in admin panel.'
            })

        return jsonify({
            'success': True,
            'ads': selected_ads,
            'total_ads': len(selected_ads),
            'sequence': 'popunder → banner → native_banner',
            'message': f'Selected {len(selected_ads)} ads in priority sequence'
        })
...
@app.route('/admin/new-ad-management')
def admin_new_ad_management():
    if not is_admin():
        flash('Access denied. Admin privileges required.')
        return redirect(url_for('home'))

    # Get ads by network and type
    propeller_ads = AdNetwork.query.filter_by(network_name='Propeller').order_by(AdNetwork.ad_type, AdNetwork.display_order).all()
    adsterra_ads = AdNetwork.query.filter_by(network_name='Adsterra').order_by(AdNetwork.ad_type, AdNetwork.display_order).all()
    hilltop_ads = AdNetwork.query.filter_by(network_name='Hilltop').order_by(AdNetwork.ad_type, AdNetwork.display_order).all()

    return render_template('admin_new_ad_management.html', 
                         propeller_ads=propeller_ads,
                         adsterra_ads=adsterra_ads,
                         hilltop_ads=hilltop_ads)

@app.route('/admin/add-network-ad', methods=['POST'])
def add_network_ad():
    if not is_admin():
        flash('Access denied. Admin privileges required.')
        return redirect(url_for('home'))
    ...
    if not network_name or not ad_type or (not ad_code and not ad_url):
        flash('Please fill all required fields.')
        return redirect(url_for('admin_new_ad_management'))

    try:
        # Get next display order
        max_order = db.session.query(db.func.max(AdNetwork.display_order)).filter_by(
            network_name=network_name, ad_type=ad_type
        ).scalar() or 0
        ...
        flash(f'{network_name} {ad_type} ad added successfully!')

    except Exception as e:
        db.session.rollback()
        flash(f'Error adding ad: {str(e)}')

    return redirect(url_for('admin_new_ad_management'))

@app.route('/admin/toggle-network-ad/<int:ad_id>', methods=['POST'])
def toggle_network_ad(ad_id):
    if not is_admin():
        flash('Access denied. Admin privileges required.')
        return redirect(url_for('home'))

    ad = AdNetwork.query.get(ad_id)
    if not ad:
        flash('Ad not found.')
        return redirect(url_for('admin_new_ad_management'))
    ...
    try:
        ...
        flash(f'{ad.network_name} {ad.ad_type} ad {status} successfully!')

    except Exception as e:
        db.session.rollback()
        flash(f'Error updating ad: {str(e)}')

    return redirect(url_for('admin_new_ad_management'))

@app.route('/admin/delete-network-ad/<int:ad_id>', methods=['POST'])
def delete_network_ad(ad_id):
    if not is_admin():
        flash('Access denied. Admin privileges required.')
        return redirect(url_for('home'))
    ...
    try:
        ...
        flash(f'{network_name} {ad_type} ad deleted successfully!')

    except Exception as e:
        db.session.rollback()
        flash(f'Error deleting ad: {str(e)}')
...
# AdBlock Violation Tracking
class AdBlockViolation(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('user.id'))
    violation_date = db.Column(db.String(20))  # YYYY-MM-DD format
...
def complete_ad_task():
    ...
    adblock_detected = data.get('adblock_detected', False)
    if adblock_detected:
        # Log adblock violation for admin tracking
        today = datetime.now().strftime('%Y-%m-%d')
        violation = AdBlockViolation.query.filter_by(user_id=user.id, violation_date=today).first()
        
        if violation:
            violation.violation_count += 1
        else:
            violation = AdBlockViolation(
                user_id=user.id,
        ...
        return jsonify({'success': False, 'message': 'AdBlock detected. Please disable your adblocker to earn coins.'}), 400

    # Verify minimum watch time (30 seconds)
    ...
    if time_watched < 28:  # Allow 2 seconds tolerance
        return jsonify({
            'success': False, 
            'message': f'Ad not watched completely. Watched: {int(time_watched)}s, Required: 30s'
        }), 400
    ...
    # Fixed reward for all users (0.5 coins)
    ad_reward = 0.5
    device_type = data.get('device_type', session.get('device_type', 'browser'))

    # Update ad session
    ad_session.ads_watched_count += 1
    ad_session.last_ad_time = datetime.utcnow()

    # Set cooldown after every 10 ads
    if ad_session.ads_watched_count % 10 == 0:
        ad_session.cooldown_until = datetime.utcnow() + timedelta(minutes=10)

    # Award coins - properly handle decimal values
    user.coins = float(user.coins) + ad_reward
    ...
    transaction = CoinTransaction(
        ...
        amount=ad_reward,
        description=f'Easy task completed - Session: {ad_session.ads_watched_count}/100'
    )
    db.session.add(transaction)

    # Update or create daily earning stats
    earning_stats = UserEarningStats.query.filter_by(user_id=user.id, date=today).first()
    if not earning_stats:
        earning_stats = UserEarningStats(
            user_id=user.id,
            date=today,
            total_coins_earned=ad_reward,
            ads_watched=1
        )
        db.session.add(earning_stats)
    else:
        earning_stats.total_coins_earned += ad_reward
        earning_stats.ads_watched += 1
        earning_stats.updated_at = datetime.utcnow()

    # Update ad performance stats if we have ad data
    ads_shown = data.get('ads_shown', [])
    for ad_info in ads_shown:
        ad_id = ad_info.get('ad_id')
        if ad_id:
            perf_stats = AdPerformanceStats.query.filter_by(ad_network_id=ad_id, date=today).first()
            if not perf_stats:
                perf_stats = AdPerformanceStats(
                    ad_network_id=ad_id,
                    date=today,
                    impressions=1,
                    earnings_generated=ad_reward / len(ads_shown),
                    unique_users=1
                )
                db.session.add(perf_stats)
            else:
                perf_stats.impressions += 1
                perf_stats.earnings_generated += (ad_reward / len(ads_shown))
                perf_stats.updated_at = datetime.utcnow()
    ...
    session.pop('device_type', None)

    return jsonify({
        'success': True, 
        'message': f'✅ +{ad_reward} Coins Earned Successfully!', 
        'new_balance': float(user.coins),
        'ads_watched': ad_session.ads_watched_count,
        'ads_remaining': 100 - ad_session.ads_watched_count,
        'reward_amount': ad_reward,
        'device_type': device_type,
        'in_cooldown': ad_session.ads_watched_count % 10 == 0,
        'daily_earnings': float(earning_stats.total_coins_earned)
    })
...
def api_update_order_status(order_id):
    ...
    if update_order_from_jap(order):
        ...
        if order.quantity > 0 and order.start_count > 0:
            progress_percentage = min(100, (order.start_count / order.quantity) * 100)
        ...
        return jsonify({
            ...
            'updated_at': order.last_status_update.strftime('%Y-%m-%d %H:%M:%S') if order.last_status_update else None
        })
    else:
        return jsonify({'success': False, 'message': 'Failed to update order status from JAP API.'}), 500

@app.route('/api/update-all-orders', methods=['POST'])
def api_update_all_orders():
    """API endpoint to update all pending/processing orders for current user"""
    if 'user_id' not in session:
        return jsonify({'success': False, 'message': 'Please log in first.'}), 401
    ...
    # Get all incomplete orders for the user
    incomplete_orders = Order.query.filter_by(user_id=user.id).filter(
        Order.status.in_(['Pending', 'Processing', 'Partial'])
    ).filter(Order.jap_order_id.isnot(None)).all()
...
def admin_panel():
    ...
    users_with_stats = []
    users_with_levels = []  # For template compatibility
    today = datetime.now().strftime('%Y-%m-%d')
    
    for user in users:
        user_level = get_user_level(user.id)
        
        # Get today's ad session with proper error handling
        try:
            ...
            print(f"Error getting ad session for user {user.id}: {e}")
            ads_watched_today = 0
            direct_ads_watched_today = 0
        ...
        # Add to users_with_levels for template compatibility
        users_with_levels.append((user, user_level))
...
@app.route('/admin/adblock-violations')
def admin_adblock_violations():
    if not is_admin():
        flash('Access denied. Admin privileges required.')
        return redirect(url_for('home'))

    # Get all violations with user information
    violations = db.session.query(AdBlockViolation, User).join(User, AdBlockViolation.user_id == User.id).order_by(AdBlockViolation.created_at.desc()).all()

    # Get statistics
    total_violations = AdBlockViolation.query.count()
    today = datetime.now().strftime('%Y-%m-%d')
    today_violations = AdBlockViolation.query.filter_by(violation_date=today).count()
    repeat_offenders = db.session.query(AdBlockViolation.user_id).group_by(AdBlockViolation.user_id).having(db.func.sum(AdBlockViolation.violation_count) > 10).count()
    ...
    return render_template('admin_adblock_violations.html', 
...
@app.route('/admin/help-videos')
def admin_help_videos():
    if not is_admin():
        flash('Access denied. Admin privileges required.')
        return redirect(url_for('home'))

    videos = HelpVideo.query.order_by(HelpVideo.display_order, HelpVideo.created_at.desc()).all()
    return render_template('admin_help_videos.html', videos=videos)

@app.route('/admin/add-help-video', methods=['POST'])
def add_help_video():
    if not is_admin():
        flash('Access denied. Admin privileges required.')
        return redirect(url_for('home'))
    ...
    video_type = request.form.get('video_type', 'tutorial')
    display_order = int(request.form.get('display_order', 0))
    show_on_dashboard = 'show_on_dashboard' in request.form
    ...
    if not title or not video_url:
        flash('Please fill all required fields.')
        return redirect(url_for('admin_help_videos'))
    ...
    try:
        ...
        flash(f'Help video "{title}" added successfully!')

    except Exception as e:
        db.session.rollback()
        flash(f'Error adding video: {str(e)}')

    return redirect(url_for('admin_help_videos'))

@app.route('/admin/toggle-help-video/<int:video_id>', methods=['POST'])
def toggle_help_video(video_id):
    if not is_admin():
        flash('Access denied. Admin privileges required.')
        return redirect(url_for('home'))

    video = HelpVideo.query.get(video_id)
    if not video:
    ...
    try:
        ...
        flash(f'Video "{video.title}" {status} successfully!')
...
@app.route('/admin/delete-help-video/<int:video_id>', methods=['POST'])
def delete_help_video(video_id):
    if not is_admin():
        flash('Access denied. Admin privileges required.')
        return redirect(url_for('home'))
    ...
    try:
        ...
        flash(f'Video "{title}" deleted successfully!')
...
def admin_push_notifications():
    ...
    notifications = PushNotification.query.order_by(PushNotification.created_at.desc()).all()
    return render_template('admin_push_notifications.html', notifications=notifications)

@app.route('/admin/create-push-notification', methods=['POST'])
def create_push_notification():
    ...
    send_immediately = 'send_immediately' in request.form
    ...
    try:
        ...
        flash(f'Push notification "{title}" created successfully!')
...
def api_admin_live_stats():
    ...
    try:
        ...
        try:
            total_direct_ads_watched_today = sum(getattr(session, 'direct_ads_watched_count', 0) for session in today_ad_sessions)
        except:
            total_direct_ads_watched_today = 0
        
        active_ad_watchers = len([s for s in today_ad_sessions if s.ads_watched_count > 0])
        ...
        top_users = []
        for session in sorted(today_ad_sessions, key=lambda x: x.ads_watched_count, reverse=True)[:5]:
            user = User.query.get(session.user_id)
            if user:
                top_users.append({
                    'username': user.username,
                    ...
                    'last_activity': user.last_activity.isoformat() if user.last_activity else None,
                    'status': 'online' if (datetime.utcnow() - user.last_activity).total_seconds() <= 300 else 'offline' if user.last_activity else 'never'
                })
        
        # Get earnings stats
        today_earnings = db.session.query(db.func.sum(CoinTransaction.amount)).filter(
            CoinTransaction.created_at >= datetime.now().replace(hour=0, minute=0, second=0),
            CoinTransaction.transaction_type.in_(['ad_reward', 'login_bonus'])
        ).scalar() or 0
        
        # Get network stats
        network_stats = {}
        for network in ['Propeller', 'Adsterra', 'Hilltop']:
            network_ads = AdNetwork.query.filter_by(network_name=network, is_active=True).count()
            network_stats[network.lower()] = network_ads
...
def complete_spin():
    ...
    # Define actual rewards with corrected probabilities
    # Try Again: 40%, 1 Coin: 40%, 5 Coins: 20%
    rewards = [
        {'type': 'extra_spin', 'amount': 0, 'message': 'Try Again! 🔄', 'weight': 40},
        {'type': 'coins', 'amount': 1, 'message': '1 Coin Won! 🪙', 'weight': 40},
        {'type': 'coins', 'amount': 5, 'message': '5 Coins Won! 🪙', 'weight': 20}
    ]
    ...
    # Sync updated data to Supabase after coin changes
    if selected_reward['type'] == 'coins' and selected_reward['amount'] > 0:
        sync_user_data_to_supabase(user.id)
...
def test_db():
    ...
    try:
        ...
        # Test Supabase API data retrieval
        supabase_data = []
        supabase_stats = {}
        try:
            # Get Supabase user activity data
            result = supabase.table('user_activity').select('*').limit(10).execute()
            supabase_data = result.data
            
            # Get Supabase statistics
            stats_result = supabase.table('user_activity').select('*', count='exact').execute()
            supabase_stats = {
                'total_records': len(stats_result.data),
                'online_users': len([u for u in stats_result.data if u.get('is_online', False)]),
                'last_sync': max([u.get('sync_timestamp', '') for u in stats_result.data]) if stats_result.data else 'None'
            }
        except Exception as e:
            supabase_data = f"Error: {str(e)}"
        ...
        <h3>Supabase API Data (Latest 10 records):</h3>
        <pre>{db_info['supabase_data']}</pre>
        
        <h3>Supabase Statistics:</h3>
        <ul>
        <li>Total Records: {db_info['supabase_stats'].get('total_records', 'N/A')}</li>
        <li>Online Users: {db_info['supabase_stats'].get('online_users', 'N/A')}</li>
        <li>Last Sync: {db_info['supabase_stats'].get('last_sync', 'N/A')}</li>
        </ul>
...
def help_videos():
    ...
    videos = HelpVideo.query.filter_by(is_active=True).order_by(HelpVideo.display_order, HelpVideo.created_at.desc()).all()

    return render_template('help_videos.html', user=user, videos=videos)
...
def watch_video(video_id):
    ...
    video = HelpVideo.query.get_or_404(video_id)
    
    # Increment view count
    video.views_count += 1
    db.session.commit()

    return render_template('watch_video.html', user=user, video=video)

@app.route('/api/get-help-videos')
def api_get_help_videos():
    """API endpoint to get active help videos"""
    try:
        videos = HelpVideo.query.filter_by(is_active=True).order_by(HelpVideo.display_order, HelpVideo.created_at.desc()).all()
        ...
        for video in videos:
            ...
            video_id = video.video_url.split('v=')[-1].split('&')[0] if 'youtube.com' in video.video_url else video.video_url.split('/')[-1]
...
if __name__ == '__main__':
    ...
    debug_mode = not Config.IS_RENDER  # Disable debug in production
