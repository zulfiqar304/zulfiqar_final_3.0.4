...
def admin_panel():
    ...
    # Add level information to users
    users_with_levels = []
    for user in users:
    ...
    orders = db.session.query(Order, User).join(User, Order.user_id == User.id).order_by(Order.id.asc()).all()

    today = datetime.now().strftime('%Y-%m-%d')

    return render_template('admin_panel.html', users_with_levels=users_with_levels, orders=orders, search_query=search_query, today=today, datetime=datetime)
